...
/// The `NetReachability` class listens for reachability changes of hosts and addresses for both WWAN and
...
open class NetReachability {
    ...
    /// Creates a `NetReachability` instance with the specified host.
    ///
    /// - parameter host: The host used to evaluate network reachability.
    ///
    /// - returns: The new `NetReachability` instance.
    public convenience init?(host: String) {
        guard let reachability = SCNetworkReachabilityCreateWithName(nil, host) else { return nil }
        self.init(reachability: reachability)
    }
    
    /// Creates a `NetReachability` instance that monitors the address 0.0.0.0.
    ///
    ...
    func reachabilityStatusForFlags(_ flags: SCNetworkReachabilityFlags) -> NetReachabilityStatus {
        guard isNetworkReachable(with: flags) else { return .notReachable }
        
        var networkStatus: NetReachabilityStatus = .reachable(.ethernetOrWiFi)
...
extension NetReachabilityStatus: Equatable {}
...
public func ==(lhs: NetReachabilityStatus, rhs: NetReachabilityStatus) -> Bool
{
    switch (lhs, rhs) {
