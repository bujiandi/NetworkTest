...
extension NSDate {
    // MARK: - 可以获取时间差
    public static func -(lhs: NSDate, rhs: NSDate) -> TimeInterval {
        return lhs.timeIntervalSince1970 - rhs.timeIntervalSince1970
    }
    public static func -(lhs: NSDate, rhs: Date) -> TimeInterval {
        return lhs.timeIntervalSince1970 - rhs.timeIntervalSince1970
    }
}
...
extension Date {
    ...
    public init?(_ v: String, style: DateFormatter.Style) {
        ...
        if let date = formatter.date(from: v) {
            ...
            if let date = formatter.date(from: v) {
                ...
                #if DEBUG
                    fatalError("日期字符串格式异常[\(v)] at line:\(#line) at column:\(#column)")//__FILE__,__FUNCTION__
                #else
                    return nil
                #endif
            }
...
extension Date : ExpressibleByStringLiteral {
    ...
    public init(extendedGraphemeClusterLiteral value: ExtendedGraphemeClusterLiteralType) {
        self = Date(value) ?? Date(timeIntervalSince1970: 0)
    }
    public init(unicodeScalarLiteral value: UnicodeScalarLiteralType) {
        self = Date(value.description) ?? Date(timeIntervalSince1970: 0)
    }
...
extension TimeInterval {
    public var date:Date { return Date(timeIntervalSince1970: self) }
}
